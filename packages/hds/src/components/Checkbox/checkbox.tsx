  ...
  (props, ref) => {
    ...
    const checkboxIcon = () => {
      if (
        ...
        return <MinusIcon data-testid={`${uid}-minus-icon`} />;
      }
      return <CheckboxIcon data-testid={`${uid}-check-icon`} />;
    };
    ...
    const radioIcon = () => {
      ...
      return (
        <CircleIcon fontSize={iconSize} data-testid={`${uid}-circle-icon`} />
      );
    };
      ...
      <React.Fragment>
        {props.type === 'checkboxType' && (
          <ChakraCheckbox
            ...
            isDisabled={disabled}
            sx={classHover}
            data-testid={props.__testId ?? `${uid}-checkbox`}
            onChange={({ target }) => {
  ...
  return (
    <VStack align="start" justify="start" spacing={0}>
      <Text
        ...
        color="neutrals.900"
        data-testid={`${uid}-checkbox-label`}
      >
        {checkboxLabel}
      </Text>
      {helperMsg && (
        <Text
          size="label-xs-default"
          color="neutrals.600"
          data-testid={`${uid}-checkbox-helperMsg`}
        >
          {helperMsg}
        </Text>
      )}
